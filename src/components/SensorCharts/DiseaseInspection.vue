<template>
  <div class="disease-inspection">
    <svg :width="svgWidth" :height="svgHeight" @mousemove="handleMouseMove">
      <!-- 箭头标记定义 -->
      <defs>
        <marker id="arrow" markerWidth="10" markerHeight="10" refX="19" refY="-135" orient="auto">
      ...
      <g v-for="(lane, index) in plantingLanes" :key="'lane-' + index">
        <rect :x="lane.x" :y="lane.y" :width="lane.width" :height="lane.height" fill="rgba(114, 177, 118, 0.1)"
          stroke="#72b176" stroke-width="2" rx="4" />
      </g>

      <!-- 巡检路径 -->
      <path v-for="(path, index) in inspectionPaths" :key="'path-' + index" :d="generatePathD(path)" fill="none"
        stroke="#4a90e2" stroke-width="2" stroke-dasharray="5 3" marker-end="url(#arrow)" />

      <!-- 检测点 -->
      <circle v-for="(point, index) in detectionPoints" :key="'point-' + index" :cx="point.cx" :cy="point.cy" r="4"
        fill="#ff6b6b" class="detection-point" @mouseover="handlePointHover(point, $event)" @mouseout="hideTooltip" />

      <!-- 信息提示框 -->
      <foreignObject v-if="activePoint" :x="Math.min(tooltipX, svgWidth - 210)" :y="Math.min(tooltipY, svgHeight - 110)"
        width="200" height="200" class="tooltip">
